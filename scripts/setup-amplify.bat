@echo off
REM Huur-US AWS Amplify Setup Script for Windows
...
set PROJECT_NAME=huur-us
set ENVIRONMENT=prod

echo [INFO] Starting AWS Amplify setup for Huur-US...
...
for /f "tokens=*" %%i in ('aws amplify create-app --name "%PROJECT_NAME%" --description "Huur-US Toll Management Platform" --platform "WEB" --repository "https://github.com/your-org/huur-us-react" --region %AWS_REGION% --query "app.appId" --output text') do set APP_ID=%%i
...
if /i "%SETUP_DOMAIN%"=="y" (
    set /p CUSTOM_DOMAIN="Enter your custom domain (e.g., huur-us.com): "
...
echo.
echo [SUCCESS] 🎉 AWS Amplify setup completed successfully!
echo.
echo Setup Summary:
echo ==============
echo App ID: %APP_ID%
...
echo ================
echo • View app: aws amplify get-app --app-id %APP_ID% --region %AWS_REGION%
echo • List branches: aws amplify list-branches --app-id %APP_ID% --region %AWS_REGION%
echo • View jobs: aws amplify list-jobs --app-id %APP_ID% --branch-name main --region %AWS_REGION%
echo.
