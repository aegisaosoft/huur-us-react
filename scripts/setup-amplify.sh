...
# Huur-US AWS Amplify Setup Script
...
create_amplify_app() {
    ...
    APP_ID=$(aws amplify create-app \
        ...
        --platform "WEB" \
        --repository "https://github.com/your-org/huur-us-react" \
        --region ${AWS_REGION} \
        --query 'app.appId' \
        --output text)
...
setup_custom_domain() {
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        read -p "Enter your custom domain (e.g., huur-us.com): " CUSTOM_DOMAIN
...
display_summary() {
    log_success "🎉 AWS Amplify setup completed successfully!"
    echo ""
    echo "Setup Summary:"
    echo "=============="
    echo "App ID: ${APP_ID}"
...
# Main setup function
main() {
    log_info "Starting AWS Amplify setup for Huur-US..."
...
case "${1:-setup}" in
    ...
    "help"|"-h"|"--help")
        ...
        echo "  PROJECT_NAME - Project name (default: huur-us)"
