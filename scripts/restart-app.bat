@echo off
REM Huur-US Application Restart Script for Windows
REM This script restarts the Huur-US application with proper service management

setlocal enabledelayedexpansion

REM Configuration
if "%SERVICE_NAME%"=="" set SERVICE_NAME=Huur-US Application
if "%APP_PORT%"=="" set APP_PORT=5000
if "%DEPLOY_DIR%"=="" set DEPLOY_DIR=C:\inetpub\wwwroot\huur-us
if "%SERVER_DIR%"=="" set SERVER_DIR=%DEPLOY_DIR%\server

echo [INFO] Starting Huur-US application restart process...
...
echo [INFO] 🛑 Stopping Huur-US application...
...
echo [INFO] 🔄 Starting Huur-US application...
...
echo [SUCCESS] 🎉 Application restart completed successfully!
...
echo   description: 'Huur-US Toll Management Platform', >> "%SERVER_DIR%\install-service.js"
