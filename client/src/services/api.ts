...
api.interceptors.request.use((config) => {
  ...
  // Add API environment header (for switching between Huur APIs)
  const apiEnvironment = localStorage.getItem('huur-environment') || 'development';
...
export const huurApi = {
  signin: async (credentials: { email: string; password: string }) => {
    const response = await api.post('/huur/signin', credentials);
    return response.data;
  },
  
  getViolations: async (params?: { licensePlate?: string; state?: string }) => {
    const response = await api.get('/huur/violations', { params });
    return response.data;
  },
  
  getEzPass: async (dateFrom?: string, dateTo?: string, page?: number) => {
    // Use retry mechanism for EzPass data fetching
    const response = await retryWithBackoff(() => api.get('/huur/ezpass', {
  ...
  exportEzPassExcel: async (dateFrom: string, dateTo: string) => {
    ...
    const response = await retryWithBackoff(() => api.post('/huur/ezpass/export/excel', { dateFrom, dateTo }, {
      responseType: 'blob'
    }));
    return response.data;
  },
  ...
  exportEzPassPdf: async (dateFrom: string, dateTo: string) => {
    ...
    const response = await retryWithBackoff(() => api.post('/huur/ezpass/export/pdf', { dateFrom, dateTo }, {
      responseType: 'blob'
    }));
    return response.data;
  },
  ...
  getParkingViolations: async (dateFrom?: string, dateTo?: string, page?: number, ownerId?: string) => {
    ...
    const response = await api.get('/huur/parking-violations', {
      params
    });
    return response.data;
  },
  ...
  exportParkingViolationsExcel: async (dateFrom: string, dateTo: string) => {
    const response = await api.post('/huur/parking-violations/export/excel', { dateFrom, dateTo }, {
      responseType: 'blob'
    });
    return response.data;
  },
  ...
  exportParkingViolationsPdf: async (dateFrom: string, dateTo: string) => {
    const response = await api.post('/huur/parking-violations/export/pdf', { dateFrom, dateTo }, {
      responseType: 'blob'
    });
    return response.data;
  },
  ...
  getNYCViolations: async () => {
    const response = await api.get('/huur/nyc-violations');
    return response.data;
  },
  
  getPayments: async (type?: 'completed' | 'failed') => {
    const endpoint = type ? `/huur/payments/${type}` : '/huur/payments/completed';
  ...
  createViolation: async (violationData: any) => {
    ...
    console.log('URL:', '/huur/violations');
    console.log('Body:', violationData);
    const response = await api.post('/huur/violations', violationData);
    console.log('=== API SERVICE CREATE VIOLATION RESPONSE ===', response.status, response.data);
    return response.data;
  },

  deleteViolation: async (id: string) => {
    const response = await api.delete(`/huur/violation/delete/${id}`);
    return response.data;
  },

  getViolationById: async (id: string) => {
    const response = await api.get(`/huur/violation/get/${id}`);
    return response.data;
  },
  ...
  updateViolation: async (id: string, violationData: any) => {
    ...
    console.log('URL:', `/huur/violation/update/${id}`);
    
    const response = await api.put(`/huur/violation/update/${id}`, violationData);
  ...
  getTolls: async (dateFrom?: string, dateTo?: string, page?: number, ownerId?: string) => {
    ...
    const response = await api.get('/huur/tolls', {
      params
    });
    return response.data;
  },
  ...
  getPendingPayments: async (dateFrom?: string, dateTo?: string, page?: number, ownerId?: string) => {
    ...
    const response = await api.get('/huur/pending-payments', {
      params
    });
    return response.data;
  },
  ...
  getActiveCompanies: async () => {
    const response = await api.get('/huur/Companies/active');
    return response.data;
  },

  createCar: async (carData: CreateCarRequest) => {
    const response = await api.post('/huur/cars', carData);
    return response.data;
  },

  getCars: async (role: number = 0) => {
    const response = await api.post('/huur/cars/carlist', { role });
    return response.data;
  },
  ...
  getExternalTollDailyInvoice: async (dateFrom?: string, dateTo?: string, ownerId?: string) => {
    ...
    const response = await api.get('/huur/external-daily-invoice', { params });
    return response.data;
  },

  createExternalTollDailyInvoice: async (tollData: any) => {
    const response = await api.post('/huur/external-daily-invoice/create', tollData);
    return response.data;
  },

  updateExternalTollDailyInvoice: async (id: string, tollData: any) => {
    const response = await api.put(`/huur/external-daily-invoice/update/${id}`, tollData);
    return response.data;
  },

  getExternalTollDailyInvoiceById: async (id: string) => {
    const response = await api.get(`/huur/external-daily-invoice/get/${id}`);
    return response.data;
  },
  ...
  getInvoice: async (dateFrom?: string, dateTo?: string, companyId?: string) => {
    ...
    const response = await api.get('/huur/invoice', {
      params
    });
    return response.data;
  },
  ...
  getInvoiceDetails: async (dateFrom?: string, companyId?: string) => {
    ...
    const response = await api.get('/huur/invoice-details', {
      params
    });
    return response.data;
  },
  ...
  getInvoiceByCompany: async (companyId: string, date?: string) => {
    ...
    try {
      const response = await api.get(`/huur/invoice-by-company/${companyId}`, {
        params
      });
  ...
  submitInvoice: async (invoiceId: string) => {
    const response = await api.post(`/huur/invoice-submit/${invoiceId}`);
    return response.data;
  },

  failInvoice: async (invoiceId: string) => {
    const response = await api.post(`/huur/invoice-fail/${invoiceId}`);
    return response.data;
  },

  getInvoiceDetailsById: async (invoiceId: string) => {
    const response = await api.get(`/huur/invoice-details-by-id/${invoiceId}`);
    return response.data;
  },

  deleteExternalTollDailyInvoice: async (id: string) => {
    const response = await api.delete(`/huur/external-daily-invoice/delete/${id}`);
    return response.data;
  },
  ...
  updateExternalTollDailyInvoicePaymentStatus: async (ids: string[], paymentStatus: number, companyId?: string) => {
    ...
    const response = await api.put('/huur/external-daily-invoice/update-payment-status', {
      ids,
      paymentStatus,
      companyId
    }, { 
  ...
  updateViolationPaymentStatus: async (ids: number[], paymentStatus: number, companyId?: string) => {
    ...
    const response = await api.put('/huur/violation/update-payment-status', {
      ids,
      paymentStatus,
      companyId
    }, {
  ...
  updateInvoiceStatus: async (invoiceId: string, status: number) => {
    ...
    const response = await api.put('/huur/external-daily-invoice/update-status', {
      invoiceId,
      status
    }, {
      timeout: 30000 // 30 second timeout
  ...
  getAllExternalDailyInvoices: async (dateFrom?: string, dateTo?: string, companyId?: string) => {
    ...
    const response = await api.get('/huur/external-daily-invoice/get-all', {
      params,
      timeout: 30000 // 30 second timeout
    });
  ...
  getExternalDailyInvoiceDetails: async (invoiceId: string) => {
    ...
    const response = await api.get(`/huur/external-daily-invoice/get-details/${invoiceId}`, {
      timeout: 30000 // 30 second timeout
    });
