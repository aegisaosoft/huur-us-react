...
const ParkingViolations: React.FC = () => {
  ...
  const { data: violationData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => huurApi.getParkingViolations(dateFrom, dateTo, 1, selectedCompanyId),
  ...
  //       try {
  //         await huurApi.deleteViolation(violationId.toString());
  ...
  const handleSubmitSelected = async () => {
    ...
    try {
      ...
      await huurApi.updateViolationPaymentStatus(
        selectedRows,
        paymentStatus,
