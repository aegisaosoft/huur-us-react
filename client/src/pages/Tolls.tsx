...
const Tolls: React.FC = () => {
  ...
  const { data: tollsData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => {
      return huurApi.getTolls(dateFrom, dateTo, 1, selectedCompanyId || '');
    },
  ...
  useEffect(() => {
    ...
    // Map HuurApi ExternalTollDailyInvoice format to frontend TollRecord format
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const tableData = filteredData.map((row: any) => [
        ...
        row.paymentStatus === 0 ? 'Paid' : row.paymentStatus === 1 ? 'Unpaid' : row.paymentStatus === 2 ? 'Processing' : 'Unknown',
      ...
      try {
        ...
        filteredData.forEach((row: any, index: number) => {
          ...
          const rowData = [
            ...
            row.amount ? `$${parseFloat(row.amount).toFixed(2)}` : 'N/A',
            (row.paymentStatus === 0 ? 'Paid' : row.paymentStatus === 1 ? 'Unpaid' : row.paymentStatus === 2 ? 'Processing' : 'Unknown').substring(0, 8),
  ...
  const _handleDeleteSelected = async () => {
    ...
    try {
      ...
      const deletePromises = selectedRows.map(async (tollId) => {
        try {
          await huurApi.deleteExternalTollDailyInvoice(tollId);
  ...
  const handleSubmitSelected = async () => {
    ...
    try {
      ...
      await huurApi.updateExternalTollDailyInvoicePaymentStatus(
        selectedRows,
        paymentStatus,
