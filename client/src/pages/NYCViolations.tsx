...
import { huurApi, default as api } from '../services/api';
...
const NYCViolations: React.FC = () => {
  ...
  const { data: carsResponse, isLoading: carsLoading, error: carsError, refetch: refetchCars } = useQuery({
    ...
    queryFn: () => huurApi.getCars(0), // role = 0
    enabled: hasSearched,
    retry: 1
  });
    ...
    const fetchViolations = async () => {
      ...
      try {
        ...
        try {
          ...
          const url = `/huur/nyc-violations?licensePlates=${encodeURIComponent(JSON.stringify(licensePlates))}&dateFrom=${encodeURIComponent(formattedDateFrom)}&dateTo=${encodeURIComponent(formattedDateTo)}&limit=5000`;
