...
const Invoice: React.FC = () => {
    ...
    queryFn: async () => {
      try {
        ...
        if (urlCompanyId && urlDate) {
          ...
          result = await huurApi.getInvoiceByCompany(urlCompanyId, urlDate);
        }
        // Priority 2: invoiceId - use invoice details
        else if (invoiceId) {
          console.log('Fetching invoice by ID:', invoiceId);
          result = await huurApi.getExternalDailyInvoiceDetails(invoiceId);
        ...
        // Priority 3: Manual search - use invoice by company
          ...
          result = await huurApi.getInvoiceByCompany(selectedCompanyId, dateFrom);
        }
  ...
  const updateInvoiceStatusMutation = useMutation({
    mutationFn: ({ invoiceId, status }: { invoiceId: string; status: number }) => 
      huurApi.updateInvoiceStatus(invoiceId, status),
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  useMutation({
    mutationFn: (invoiceId: string) => huurApi.submitInvoice(invoiceId),
    onSuccess: async (data) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  useMutation({
    mutationFn: (invoiceId: string) => huurApi.failInvoice(invoiceId),
    onSuccess: async (data) => {
