...
const HUUR_API_CONFIG = {
  ...
  apiKey: process.env.HUUR_API_KEY || ''
};
...
router.post('/login', async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('Attempting login with:', { email, apiBaseUrl, hasApiKey: !!HUUR_API_CONFIG.apiKey });
      
      // Call real HuurApi authentication
      const response = await axios.post(`${apiBaseUrl}/UserAuth/signin`, {
        ...
        headers: {
          ...
          'X-API-Key': HUUR_API_CONFIG.apiKey
        },
        timeout: 30000
      });
      
      console.log('HuurApi response status:', response.status);

      const apiResponse = response.data;
      console.log('HuurApi response data:', apiResponse);
      ...
      if (apiResponse.reason === 0 && apiResponse.result) {
        ...
        const user = {
          ...
          imageURL: apiResponse.result.imageURL || '/images/huur-logo.png', // Use real ImageURL from API
          huurApiToken: apiResponse.result.token
        };

        // Store user data AND token in session
        req.session.user = user;
        req.session.huurApiToken = user.huurApiToken;

        console.log('Session created with user:', user.email);
        console.log('Token stored in session:', user.huurApiToken ? 'Yes' : 'No');

        // Return response in format expected by frontend
        res.json({
          success: true,
          token: user.huurApiToken,
          user: {
            id: user.id,
            email: user.email,
      ...
      console.error('HuurApi authentication error:', {
      ...
      // DEVELOPMENT ONLY: Create a mock HuurApi token for fallback authentication
      const token = jwt.sign(
        { 
          userId: user.id, 
          email: user.email,
          isOwner: user.isOwner,
          type: 'mock-huur-token'
        },
        process.env.JWT_SECRET || 'huur-us-jwt-secret-change-in-production',
        { expiresIn: '24h' }
      );
...
router.get('/me', async (req, res) => {
  ...
  if (token) {
    ...
    if (req.session.user && req.session.huurApiToken === token) {
      console.log('Returning session user with matching token');
      return res.json({ user: req.session.user });
    }
    
    // If no session or token doesn't match, validate token with HuurApi
    try {
      const apiBaseUrl = getApiBaseUrl(req);
      console.log('Validating token with HuurApi:', apiBaseUrl);
      
      // Call HuurApi to validate token and get user info
      ...
      if (response.data && response.data.result) {
        ...
        req.session.huurApiToken = token;
...
const verifyToken = (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'huur-us-jwt-secret-change-in-production');
    req.user = decoded;
    next();
  } catch (error) {
