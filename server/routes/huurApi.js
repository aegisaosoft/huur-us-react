...
// HuurApi configuration
const HUUR_API_CONFIG = {
  baseUrl: process.env.HUUR_API_BASE_URL || 'https://agsm-back.azurewebsites.net',
  baseUrlDev: process.env.HUUR_API_BASE_DEV_URL || 'https://agsm-back.azurewebsites.net',
  baseUrlProd: process.env.HUUR_API_BASE_URL_PROD || 'https://agsm-huur-production-api.azurewebsites.net',
  apiKey: process.env.HUUR_API_KEY || 'your-huur-api-key-here'
};
...
function getApiBaseUrl(req) {
  ...
  switch (environment) {
    case 'production':
      selectedUrl = HUUR_API_CONFIG.baseUrlProd;
      break;
    case 'development':
    default:
      selectedUrl = HUUR_API_CONFIG.baseUrlDev;
      break;
  }
...
// Create axios instance for HuurApi (default)
const huurApiClient = axios.create({
  baseURL: HUUR_API_CONFIG.baseUrl,
  timeout: 30000,
  headers: {
...
function createHuurApiClient(req) {
  return axios.create({
...
async function getWithFallbacks(req, paths, options) {
  ...
  const client = createHuurApiClient(req);
...
// Helper to forward Authorization header from client to HuurApi
function getForwardAuthHeaders(req) {
  ...
  console.log('Session token:', !!req.session?.huurApiToken);
  
  // First priority: use token from session (most reliable)
  if (req.session?.huurApiToken) {
    console.log('Using token from session');
    headers.Authorization = `Bearer ${req.session.huurApiToken}`;
  }
  ...
  else if (HUUR_API_CONFIG.apiKey && HUUR_API_CONFIG.apiKey !== 'your-huur-api-key-here') {
    console.log('Using API key as fallback');
    headers.Authorization = `Bearer ${HUUR_API_CONFIG.apiKey}`;
  } else {
...
router.post('/signin', async (req, res) => {
  try {
    ...
    console.error('HuurApi signin error:', error);
    ...
    if (error.response) {
      ...
      res.status(500).json({ 
        reason: 1, 
        message: 'Failed to connect to HuurApi' 
      });
    }
  }
});
...
router.get('/violations', async (req, res) => {
  try {
    ...
    console.error('HuurApi violations error:', error);
...
router.get('/violation/get/:id', async (req, res) => {
  try {
    ...
    console.error('HuurApi violation get error:', error);
...
router.put('/violation/update-payment-status', async (req, res) => {
  ...
  try {
    ...
    console.log('Calling HuurApi endpoint: /ExternalViolation/update-payment-status');
    console.log('Request payload:', { ids, paymentStatus, companyId });
    
    const client = createHuurApiClient(req);
    const response = await client.put('/ExternalViolation/update-payment-status', {
      ids,
      paymentStatus,
      companyId
    }, {
    ...
    console.error('HuurApi update violation payment status error:', error);
...
router.put('/violation/update/:id', async (req, res) => {
  try {
    ...
    console.error('HuurApi violation update error:', error);
    console.error('Error details:', {
      message: error.message,
      status: error.response?.status,
      statusText: error.response?.statusText,
...
router.delete('/violation/delete/:id', async (req, res) => {
  try {
    ...
    console.error('HuurApi violation delete error:', error);
...
router.get('/ezpass', async (req, res) => {
  try {
    ...
    console.error('HuurApi EZ Pass error:', error);
...
router.post('/ezpass/export/excel', async (req, res) => {
  try {
    ...
    console.error('HuurApi EZ Pass Excel export error:', error);
...
router.post('/ezpass/export/pdf', async (req, res) => {
  try {
    ...
    console.error('HuurApi EZ Pass PDF export error:', error);
...
router.get('/parking-violations', async (req, res) => {
  try {
    ...
    console.log('Base URL:', HUUR_API_CONFIG.baseUrl);
    ...
    console.error('HuurApi parking violations error:', error);
...
router.post('/parking-violations/export/excel', async (req, res) => {
  try {
    ...
    console.error('HuurApi parking violations Excel export error:', error);
...
router.post('/parking-violations/export/pdf', async (req, res) => {
  try {
    ...
    console.error('HuurApi parking violations PDF export error:', error);
...
router.get('/tolls', async (req, res) => {
  try {
    ...
    console.error('HuurApi tolls error:', error);
...
router.get('/invoice-details', async (req, res) => {
  try {
    ...
    console.error('HuurApi invoice details error:', error);
...
router.get('/invoice-by-company/:companyId', async (req, res) => {
  try {
    ...
    const response = await createHuurApiClient(req).get(endpoint, {
      headers: getForwardAuthHeaders(req)
    });
    ...
    console.error('HuurApi invoice by company error:', error);
...
router.get('/invoice-details-by-id/:invoiceId', async (req, res) => {
  try {
    ...
    console.error('HuurApi get invoice details error:', error);
...
router.post('/invoice-submit/:invoiceId', async (req, res) => {
  try {
    ...
    const response = await createHuurApiClient(req).post(endpoint, {}, {
      headers: getForwardAuthHeaders(req)
    });
    ...
    console.error('HuurApi submit invoice error:', error);
...
router.post('/invoice-fail/:invoiceId', async (req, res) => {
  try {
    ...
    console.error('HuurApi fail invoice error:', error);
...
router.get('/pending-payments', async (req, res) => {
  try {
    ...
    console.error('HuurApi pending payments error:', error);
...
router.get('/payments/:status', async (req, res) => {
  try {
    ...
    console.error('HuurApi payments error:', error);
...
router.get('/Companies/active', async (req, res) => {
  try {
    ...
    console.error('HuurApi companies error:', error);
    console.error('Error details:', {
      message: error.message,
      status: error.response?.status,
      statusText: error.response?.statusText,
...
router.post('/cars/carlist', async (req, res) => {
  try {
    ...
    console.error('HuurApi cars list error:', error);
    console.error('Error details:', {
      message: error.message,
      status: error.response?.status,
      statusText: error.response?.statusText,
...
router.get('/external-daily-invoice', async (req, res) => {
  try {
    ...
    console.log('HuurApi response received successfully');
    ...
    console.error('HuurApi External Daily Invoice error:', error);
    console.error('Error details:', {
      message: error.message,
      status: error.response?.status,
      statusText: error.response?.statusText,
...
router.post('/external-daily-invoice/create', async (req, res) => {
  try {
    ...
    const response = await createHuurApiClient(req).post('/api/ExternalTollDailyInvoice/create', tollData, {
      headers: getForwardAuthHeaders(req),
    ...
    console.error('HuurApi External Daily Invoice creation error:', error);
    console.error('Error details:', {
      message: error.message,
      status: error.response?.status,
      statusText: error.response?.statusText,
...
router.get('/external-daily-invoice/get-all', async (req, res) => {
  try {
    ...
    console.error('HuurApi get all external daily invoices error:', error);
    console.error('Error details:', {
      message: error.message,
      status: error.response?.status,
      statusText: error.response?.statusText,
...
router.get('/external-daily-invoice/get-details/:invoiceId', async (req, res) => {
  try {
    ...
    console.error('HuurApi get external daily invoice details error:', error);
    console.error('Error details:', {
      message: error.message,
      status: error.response?.status,
      statusText: error.response?.statusText,
...
router.get('/external-daily-invoice/get/:id', async (req, res) => {
  try {
    ...
    console.error('HuurApi External Daily Invoice get error:', error);
...
router.put('/external-daily-invoice/update-payment-status', async (req, res) => {
  try {
    ...
    console.log('Calling endpoint:', endpoint);
    console.log('Request payload to HuurApi:', { ids, paymentStatus, companyId: bodyCompanyId });
    console.log('Auth headers:', getForwardAuthHeaders(req));
    ...
    console.error('HuurApi update payment status error:', error);
...
router.put('/external-daily-invoice/update/:id', async (req, res) => {
  try {
    ...
    console.error('HuurApi External Daily Invoice update error:', error);
...
router.delete('/external-daily-invoice/delete/:id', async (req, res) => {
  try {
    ...
    console.error('HuurApi External Daily Invoice delete error:', error);
...
router.post('/cars', async (req, res) => {
  try {
    ...
    console.error('HuurApi car creation error:', error);
    console.error('Error details:', {
      message: error.message,
      status: error.response?.status,
      statusText: error.response?.statusText,
...
router.post('/violations', async (req, res) => {
  try {
    ...
    // Transform the data to match the Huur API expected format
    const transformedData = {
    ...
    console.log('Transformed violation data for Huur API:', transformedData);
    
    // Call the real Huur API endpoint with Bearer authentication
    const authHeaders = getForwardAuthHeaders(req);
    const bearerToken = `Bearer ${HUUR_API_CONFIG.apiKey}`;
    
    console.log('=== BEARER AUTH DEBUG ===');
    console.log('API Key:', HUUR_API_CONFIG.apiKey);
    ...
    const client = createHuurApiClient(req);
    try {
      console.log('External Huur API baseURL:', client?.defaults?.baseURL);
    ...
    console.log('Huur API response:', response.data);
    res.json(response.data);
    
  } catch (error) {
    console.error('HuurApi create violation error:', error);
...
router.put('/external-daily-invoice/update-status', async (req, res) => {
  try {
    ...
    console.error('HuurApi update invoice status error:', error);
