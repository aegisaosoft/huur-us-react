pipeline {
    ...
    environment {
        ...
        BUILD_DIR = 'build'
        DEPLOY_DIR = '/var/www/huur-us'
        SERVER_DIR = '/var/www/huur-us/server'
        CLIENT_DIR = '/var/www/huur-us/client'
        NODE_ENV = 'production'
        SERVICE_NAME = 'huur-us'
        APP_PORT = '5000'
    }
...
echo "Deploying Huur-US application..."
...
EOF
                    ...
                    cat > artifacts/huur-us.service << 'EOF'
[Unit]
...
Type=simple
User=www-data
WorkingDirectory=/var/www/huur-us/server
...
EOF
                    ...
                    cat > artifacts/nginx-huur-us.conf << 'EOF'
server {
    listen 80;
    server_name _;
    ...
    # Serve static files
    location / {
        root /var/www/huur-us/server/public;
...
EOF
        ...
        stage('Stop Application') {
            steps {
                sh '''
                    echo "🛑 Stopping Huur-US application..."
...
EOF
        ...
        stage('Start Application') {
            steps {
                sh '''
                    echo "🔄 Starting Huur-US application..."
