@echo off
REM Huur-US Production Build Script for Windows
REM This script builds the production application for Azure deployment

setlocal enabledelayedexpansion

REM Configuration
set IMAGE_NAME=huur-us
set IMAGE_TAG=production

echo === Huur-US Production Build Script ===
...
cd deploy-package
powershell Compress-Archive -Path * -DestinationPath ..\huur-us-deployment.zip -Force
cd ..
...
for %%A in (huur-us-deployment.zip) do set PACKAGE_SIZE=%%~zA
echo Package size: %PACKAGE_SIZE% bytes

echo === Build completed successfully! ===
echo Next steps:
echo 1. Deploy to Azure Web App using GitHub Actions
echo 2. Or use Azure CLI: az webapp deploy --name huur-web-us-2025 --resource-group huur_web --src-path huur-us-deployment.zip --type zip
