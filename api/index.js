...
// Huur API routes
expressApp.get('/api/huur/*', (req, res) => {
  res.json({ message: 'Huur API endpoint working', path: req.path, timestamp: new Date().toISOString() });
});

expressApp.post('/api/huur/*', (req, res) => {
  res.json({ message: 'Huur API endpoint working', path: req.path, timestamp: new Date().toISOString() });
});
