# Huur-US Windows Server Deployment Script
# This script deploys the Huur-US application to a Windows Server with IIS

param(
    [string]$Action = "deploy",
    [string]$DeployPath = "C:\inetpub\wwwroot\huur-us",
    [string]$NodeVersion = "18",
    [int]$Port = 5000
)

# Configuration
$ServiceName = "Huur-US Application"
$AppPoolName = "HuurUSAppPool"
$SiteName = "Huur-US"
...
var svc = new Service({
  name: '$ServiceName',
  description: 'Huur-US Toll Management Platform',
...
function Configure-Firewall {
    ...
    try {
        # Allow HTTP traffic
        New-NetFirewallRule -DisplayName "Huur-US HTTP" -Direction Inbound -Protocol TCP -LocalPort 80 -Action Allow | Out-Null
        New-NetFirewallRule -DisplayName "Huur-US API" -Direction Inbound -Protocol TCP -LocalPort $Port -Action Allow | Out-Null
...
"@
        ...
        $backupScript = @"
@echo off
set backup_dir=C:\Backups\Huur-US\%date:~-4,4%-%date:~-10,2%-%date:~-7,2%
...
function Start-Deployment {
    Write-Log "Starting Huur-US Windows deployment..." "SUCCESS"
    ...
    Write-Log "🌐 Application is running at http://$([System.Net.Dns]::GetHostByName($env:COMPUTERNAME).AddressList[0])/" "SUCCESS"
    Write-Log "📊 Service status: Get-Service '$ServiceName'" "SUCCESS"
    Write-Log "📝 Logs: Check Windows Event Viewer or $LogPath" "SUCCESS"
    Write-Log "🔧 Management: Use Services.msc or Get-Service command" "SUCCESS"
}
