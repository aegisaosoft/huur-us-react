...
# Huur-US Production Build Script for Azure Web App
...
echo -e "${BLUE}=== Huur-US Production Build Script ===${NC}"
...
cd deploy-package
zip -r ../huur-us-deployment.zip . -x "*.git*" "node_modules/*" "*.log"
cd ..
...
if [ $? -eq 0 ]; then
    ...
    PACKAGE_SIZE=$(du -h huur-us-deployment.zip | cut -f1)
    echo -e "${GREEN}Package size: ${PACKAGE_SIZE}${NC}"
    
    echo -e "${GREEN}=== Build completed successfully! ===${NC}"
    echo -e "${BLUE}Next steps:${NC}"
    echo -e "1. Deploy to Azure Web App using GitHub Actions"
    echo -e "2. Or use Azure CLI: az webapp deploy --name huur-web-us-2025 --resource-group huur_web --src-path huur-us-deployment.zip --type zip"
